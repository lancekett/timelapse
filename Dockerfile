FROM python:3.11-slim

# Install system dependencies (ffmpeg is required for video compilation)
RUN apt-get update && apt-get install -y --no-install-recommends \
    ffmpeg \
    curl \
    && rm -rf /var/lib/apt/lists/*

# Set up application directory
WORKDIR /app

# Copy requirements and install python dependencies
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Copy source code files and config template
COPY ai_analyzer.py archive_compiler.py compiler.py dashboard.py scheduler.py timelapse.py youtube_uploader.py weather.py catchup_uploads.py config.json ./

# Copy and setup entrypoint script
COPY timelapse-entrypoint.sh ./
RUN chmod +x timelapse-entrypoint.sh

# Expose dashboard port
EXPOSE 8000

# Set entrypoint
ENTRYPOINT ["/app/timelapse-entrypoint.sh"]
